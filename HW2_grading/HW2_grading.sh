...
function execute() {
    if [[ ! -f "$binaryName" ]]; then
        ...
        local log
        log="$(timeout "$timeLimit" time -p "./$binaryName" "$testcaseDir/$1.txt" "$outputDir/$1.out" "$2" 2>&1 >/dev/null)"
...
function verify() {
    if [[ ! -s "$outputDir/$1.out" ]]; then
        ...
        local log
        log="$("$verifyBin" "$testcaseDir/$1.txt" "$outputDir/$1.out" "$2")"
        if [[ "$log" =~ "[Error]" ]]; then
...
for testcase in "${testcasePool[@]}"; do
    for way in "2" "4"; do
        csvTitle="$csvTitle,\"$testcase $way ways cut size\",\"$testcase $way ways runtime\""
    done
done
...
for student in *; do
    if [[ -d "$student" ]]; then
        ...
        for testcase in "${testcasePool[@]}"; do
            for way in "2" "4"; do
                printf "%10s | %10s | " "$testcase" "$way"
                cutSize="N/A"
                runtime="$(execute "$testcase" "$way")"
                if [[ "$runtime" == "N/A" ]]; then
